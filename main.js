...
function drawEndingCredits() {
  ...
  textSize(25);
  text('이진우 : 비우기 파트 전체 코딩, 인터랙션 ui 방식 선정과 요소 추가 및 디자인, 전체 파트 소스코드 병합', width/2, 180);
  text('전재민 : 시작 파트 코딩, 전체 파트 디자인 애니메이션 수정, 자연물 위치선정, 각 파트별 ui디자인 수정', width/2, 260);
  text('황정민 : 채우기 파트 전체 코딩, 전체 파트의 흐름 구성 및 관리, 스토리텔링 기획, 소스코드 병합 방식 제안', width/2, 340);
